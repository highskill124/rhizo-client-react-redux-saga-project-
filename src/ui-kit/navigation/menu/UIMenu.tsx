...
const Div = styled.div`
    svg {
        *{
            fill: ${ThemeColor.danger} !important;
        }
    }
`;
    ...
    return (
        <SMenu id={`${id}-menu`} anchorEl={anchorEl} getContentAnchorEl={null} anchorOrigin={anchorOrigin} transformOrigin={transformOrigin} keepMounted open={Boolean(anchorEl)} onClose={onClose} padding={padding} margin={margin} borderRadius={borderRadius}>
            ...
            {list &&
                list.length &&
                list.map((m, i) => (
                    <MenuItem key={`${id}-menu-content-${i}`} onClick={(e) => {e.stopPropagation(); onItemClick(m)}} role="button" hasIcon={m.icon} width={width}>
                        {m.icon && m.label === 'Cancel' ? <Div><m.icon /></Div> : <div><m.icon /></div>}
                        <span>
                            {m.label === 'Cancel' ? <P>{m.label}</P> : <h4>{m.label}</h4>}
                        </span>
                    </MenuItem>
                ))}
            {/* </UIBox> */}

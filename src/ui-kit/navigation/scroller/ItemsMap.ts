...
class ItemsMap extends Map<string, IOItem> {
    ...
    public sort(arr: Item[]) {
        return arr.sort((a: Item, b: Item) => +this.onlyDigits(a[1].index) - +this.onlyDigits(b[1].index));
    }
    ...
    public filter(predicate: (value: Item, index: number, array: Item[]) => boolean): Item[] {
        return this.toArr().filter(predicate);
    }

    public find(predicate: (value: Item, index: number, obj: Item[]) => boolean): Item | undefined {
        return this.toArr().find(predicate);
    }

    public findIndex(predicate: (value: Item, index: number, obj: Item[]) => unknown): number {
